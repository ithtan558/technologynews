...
.filter('countArticleByCategory', function () {
  return function (items, article) {
    var number=0;
    if(items){
      for (var i = 0; i < items.length; i++) {
        var item = items[i];
        if (item.categoryId === article){
          number++;
        }
    }
  }
...
.filter('countArticleByTag', function () {
  return function (items, article) {
    var number=0;
    if(items){
      for (var i = 0; i < items.length; i++) {
        var item = items[i];
        if (item.tag.indexOf(article) !== -1){
          number++;
        }
    }
  }
